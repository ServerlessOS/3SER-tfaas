 ...
 * @LastEditTime: 2022-11-21 00:01:13
...
func New() ConcurrentMap {
	...
	for i := 0; i < SHARD_COUNT; i++ {
		m[i] = &ConcurrentMapShared{items: make(map[string]*pb.KeyValuePair)}
	}
	return m
}
...
func (m ConcurrentMap) MSet(data []*pb.KeyValuePair) {
 ...
 */
func (m ConcurrentMap) Set(key string, value *pb.KeyValuePair) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
	val, ok := shard.items[key]
	if ok {
		...
		if (value.GetWTimestamp() == val.GetWTimestamp()) && (value.GetRTimestamp() > val.GetRTimestamp()) {
			val.RTimestamp = value.GetRTimestamp()
		}
	} else {
		if len(value.GetValue()) != 0 {
			shard.items[key] = value
...
func (m ConcurrentMap) Upsert(key string, value *pb.KeyValuePair, cb UpsertCb) (res *pb.KeyValuePair) {
	shard := m.GetShard(key)
	shard.Lock()
...
func (m ConcurrentMap) SetIfAbsent(key string, value *pb.KeyValuePair) bool {
	// Get map shard.
	shard := m.GetShard(key)
	shard.Lock()
...
func (m ConcurrentMap) Get(key string) (*pb.KeyValuePair, bool) {
	// Get shard
	shard := m.GetShard(key)
	shard.RLock()
	// Get item from shard.
...
func (m ConcurrentMap) Pop(key string) (v *pb.KeyValuePair, exists bool) {
	// Try to get shard.
	shard := m.GetShard(key)
	shard.Lock()
