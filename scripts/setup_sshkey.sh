...
# and do not use this key beyond the scope of TFaaS experiments.

cat >> ~/.ssh/config << EOF1

Host *.compute.internal
    User ubuntu
    IdentityFile ~/.ssh/tfaas-experiments.pem
    StrictHostKeyChecking no
    UserKnownHostsFile /dev/null
EOF1

cat > ~/.ssh/tfaas-experiments.pem << EOF2
...
chmod 0600 ~/.ssh/tfaas-experiments.pem